...
let idFillRectMini = document.getElementById("fillRectMini");
...
function convertRGBToHEX(r, g, b)
{
  ...
  if (r.length < two) r = "0"+r;
  if (g.length < two) g = "0"+g;
  if (b.length < two) b = "0"+b;

  return ("#"+r+g+b).toLocaleUpperCase();
  //return ("#"+(r=="0"?"00":r)+(g=="0"?"00":g)+(b=="0"?"00":b)).toUpperCase();
}

// 
function setColor(selected="", IObj="")
{
  if (selected === "slider")
  {
    ...
    idFillRectMini.style.backgroundColor = "rgb("+r+","+g+","+b+")";
    idHEX.value = convertRGBToHEX(r, g, b);
  }
  else 
  {
    let err = {e:0};
    ...
    let obj = 
    {
      N: document.getElementById("inpNObj").value,
      ru: { title: document.getElementById("inpTitleObjRu").value },
      en: { title: document.getElementById("inpTitleObjEn").value },
      active: false
    }

    if (IObj == "")
    {
      if (obj.N.trim().length != 0 && obj.ru.title.trim().length != 0 && obj.en.title.trim().length != 0)
      {
        checkObjN(obj.N, err, "SetObj");

        if (err.e === OK+ER+EG+EB) obj.active = true;
        else
          document.getElementById("errSetObj").style.display = "block";
      }
      else
      {
        err.e = ENOTTANDNUM;
    ...
    // Зарезервирован ли этот цвет?
    for (let i = 0; i < palette.reservedColors.length; i++)
    {
      let rgb = palette.reservedColors[i];
      if (+idR.value == rgb.R && +idG.value == rgb.G && +idB.value == rgb.B)
      {
        document.getElementById("errReservedClr").style.display = "block";
        console.log("Error ReservedClr: "+(err.e=ERESERVEDCLR));
        break;
      }
    }
    ...
    {
      document.getElementById("errReservedClr").style.display = "none";
      ...
      palette.color.current = convertRGBToHEX(r, g, b);
      palette.color.R = r;
      palette.color.G = g;
      palette.color.B = b;
      ...
      if (obj.active)
      {
        ...
        setObjTitleAndNum({ N: obj.N, ru: {title: obj.ru.title}, en: {title: obj.en.title} });
      }
    }
  }
}
...
function listColorByDefalut()
{
  for (let i = zero; i < palette.colorByDefault.length; i++)
  {
    ...
    let codeHex = convertRGBToHEX(+rgb.R, +rgb.G, +rgb.B);
...
function showColorFrom(rgb)
{
  ...
  let hex = convertRGBToHEX(rgb.R, rgb.G, rgb.B)
